...
var a=b.event.__kbscopeString+(0!==b.event.local.__kbscopeString.length?".":"")+b.key,a=(p(b.args[1])?u:A)(b.event.local.__kbname,b.event.local,a);b.event.local.add(b.key,a);z(b.event.local[b.key]).parseData(b.args[1])}}function w(b){if((p(b.args[1])||n(b.args[1]))&&!v(b.args[1])){b.preventDefault();var a=b.event.__kbscopeString+(0!==b.event.local.__kbscopeString.length?".":"")+b.key,a=(p(b.args[1])?u:A)(b.event.local.__kbname,b.event.local,a);b.event.local.set(b.key,a);z(b.event.local[b.key]).parseData(b.args[1])}}
function h(b){for(var a=0,d=Object.keys(b),c=d.length;a<c;a++)this.set(d[a],b[d[a]]);return this}var m={name:B};_data=u().addPointer(m,"name","__kbname");z(_data);return _data}}(E,F);window.define&&window.require?define([],function(){return D}):window.module&&(module.exports=D);return D}();
